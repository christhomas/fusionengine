...
SimpleTank2::SimpleTank2(char *name, GameServer *server, int key): ClientBot(name,server,key)
{
	SetupTextures();
}

SimpleTank2::~SimpleTank2()
{
	delete[]	m_bot;
}

bool SimpleTank2::Initialise(void)
{
	m_size.Set(32,32,1);
	m_axis.Set(16,16,1);
...
void SimpleTank2::SetupTextures(void)
{
...
char * SimpleTank2::GetBotTexture(void)
{
	return m_bot;
}

char * SimpleTank2::GetWeaponTexture(void)
{
	return m_weapon;
}

Maths::Vector SimpleTank2::GetBotSize(void)
{
	return m_size;
}

Maths::Vector SimpleTank2::GetBotAxis(void)
{
	return m_axis;
}

bool SimpleTank2::Update()
{	
	if(m_collision==true){
		m_state = COLLISION;
		m_collision=false;
