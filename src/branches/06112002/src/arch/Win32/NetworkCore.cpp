...
extern "C" __declspec(dllexport)	INetworkCore *	GetInstance		(void);
extern "C" __declspec(dllexport)	void			DestroyInstance	(void);
...
bool NetworkCore::RemoveSocket(Socket *socket)
{
	...
	LockSockets();
	{
		...
		for(int a=0;a<m_sockets.size();a++){
			//	If you find the ptr
...
//====================================
void NetworkCore::LockSockets(void)			{	EnterCriticalSection(&m_sockets_lock);	};
void NetworkCore::UnlockSockets(void)		{	LeaveCriticalSection(&m_sockets_lock);	};
void NetworkCore::LockSendStack(void)		{	EnterCriticalSection(&m_senddata_lock);	};
void NetworkCore::UnlockSendStack(void)	{	LeaveCriticalSection(&m_senddata_lock);	};
...
DWORD WINAPI NetworkCoreThread(void *data)
{
	...
	while(true){
		...
		{
			if(event_id != 0){
				Socket *socket = network->m_network_events.m_sockets[event_id];
				...
				if(network_event.lNetworkEvents & FD_ACCEPT){
					...
					ServerSocket *server = reinterpret_cast<ServerSocket *>(socket);

					Socket *s = new Socket();
					int sockaddr_len = sizeof(s->m_socket_info);

					s->m_socket = accept(server->m_socket,(sockaddr *)&s->m_socket_info,&sockaddr_len);
					s->SetConnected(true);
					...
					int bufferlen = recv(socket->m_socket,packet->data,MAX_RECV,0);
...
void Socket::LockDataStack(void)		{	EnterCriticalSection(&m_datastack_lock);	}
void Socket::UnlockDataStack(void)	{	LeaveCriticalSection(&m_datastack_lock);	}
...
void ServerSocket::RemoveConnection(Socket *socket)
{
	if(socket != NULL){
		LockConnections();
		{
			for(SOCKETLIST::iterator s=m_Connections.begin();s!=m_Connections.end();s++){
				if((*s) == socket){
...
void ServerSocket::LockConnections(void)	{	EnterCriticalSection(&m_Connections_lock);	}
void ServerSocket::UnlockConnections(void){	LeaveCriticalSection(&m_Connections_lock);	}