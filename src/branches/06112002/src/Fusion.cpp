...
#else
	//	Include UnixModuleDB.h or something
#endif
...
void Fusion::LoadConfig(char *configfile)
{
	...
	while(config.eof()==0){
		...
		if(begin==true){
    		...
    		for(int a=0;a<numsystems;a++){
					if(m_libfilename[a] == NULL)	output << "m_filename["<<a<<"] = " << (int)m_libfilename[a] << std::endl;
    			else													output << "m_filename["<<a<<"] = " << m_libfilename[a] << std::endl;
      	}
    		output << std::endl;

        if(strcmp(command,"MODULEDB")		== 0){	m_moduledb->AddPath(param);						continue;	}
				if(strcmp(command,"GRAPHICS")		== 0){	m_libfilename[GRAPHICS]			= param;	continue;	}
				if(strcmp(command,"INPUT")			== 0){	m_libfilename[INPUT]				= param;	continue;	}
				if(strcmp(command,"SOUND")			== 0){	m_libfilename[SOUND]				= param;	continue;	}
				if(strcmp(command,"VFS")				== 0){	m_libfilename[VFS]					= param;	continue;	}
				if(strcmp(command,"NETWORK")		== 0){	m_libfilename[NETWORK]			= param;	continue;	}
...
void Fusion::InitSystem(Fusion::Subsystem id, create_t c)
{
	if(id<numsystems){
		...
		if(c != NULL){
			...
			create[id]	= (create_t)	m_moduledb->GetFunction(m_libfilename[id],"GetInstance");
			destroy[id]	= (destroy_t)	m_moduledb->GetFunction(m_libfilename[id],"DestroyInstance");
		}
