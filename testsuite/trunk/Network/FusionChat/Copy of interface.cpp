...
{
	clientMessage("/info,addUser("+user+","+id+")");
	GtkTreeIter iter;
	...
	if(findUser(user,&iter) == false){
		...
		for(useriter = usermap.begin();useriter!=usermap.end(); useriter++){
			std::stringstream sstr; sstr << "/info,user[" << a++ << "] = " << (*useriter).first << ", " << (*useriter).second;
			clientMessage(sstr.str());
		}
...
{
	//	remove a user from the treeview
	// FIXME: fusion should handle disconnects properly (it doesnt currently do anything atm)
	GtkTreeIter iter;
...
bool renameUser(std::string oldUser, std::string newUser, std::string id)
{
	...
	clientMessage("/info,Looking for "+oldUser);
	if(findUser(oldUser,&iter) == true){
		std::string test = usermap[oldUser];
		clientMessage("/info,found comparing idcodes = "+test+" / "+id);
		if(id == test){
			clientMessage("/info, id code is match, test / id = "+test+" / "+id);
			usermap.erase(usermap.find(oldUser));
			usermap[newUser] = id;
			gtk_list_store_set(ustore,&iter,0,newUser.c_str(),-1);
			
			clientMessage("/info,Shall I change username as well?");
			//	If this local client, is the owner of the requested username, change it
			if(state.username == oldUser){
				clientMessage("/info,oldUser/newUser = "+oldUser+"/"+newUser+" - changing username");
...
EXPORT void on_connect_clicked(GtkWidget *widget, gpointer data)
{
	...
	std::string state = gtk_label_get_text(connectLabel);
	
	if(state == "Connect"){
		if(server != defServerAddress){
			clientMessage("/connect,"+server);
		}else{
			clientMessage("/error,invalidAddress");
		}
	}else{
...
EXPORT void on_appMode_clicked(GtkWidget *widget, gpointer data)
{
	...
	if(text == "Client Mode"){
		...
		clientMessage("/connect,localhost");		
	}else{
...
EXPORT void on_message_activate(GtkWidget *widget, gpointer data)
{
	...
	if(text.empty() == false){
		clientMessage("/message,"+state.username+":"+text);
...
EXPORT void on_username_entry_activate(GtkWidget *widget, gpointer data)
{
	...
	clientMessage("/info,username entry activated");
	if(scolon != std::string::npos && comma != std::string::npos){
		clientMessage("/error,invalidUsername");
	}else{
